/**
  @page Dual_Combined_Reg_Inj_Simultaneous AN3116 Dual_Combined_Reg_Inj_Simultaneous Readme file
 ...
 * @version  V1.0.0
 * @date     03/09/2010
 * @brief    Description of the AN3116 Application note's 
 *           Dual_Combined_Reg_Inj_Simultaneous.
 ********************************************************************************
...
This example describes how to use the ADC in Dual Combined Regular Injected 
Simultanuous mode. 

ADC1 and ADC2 convert simultanuously regular channels. On the rising edge of the PWM 
signal generated by TIM1 the injected channels are converted simultanuously
by ADC1 and ADC2. The regular channel converted by ADC1 is channel12. The regular 
...
@par Directory contents 
  ...
  - Dual_Combined_Reg_Inj_Simultaneous/inc/stm32f10x_conf.h  Library Configuration file
  - Dual_Combined_Reg_Inj_Simultaneous/src/stm32f10x_it.c    Interrupt handlers
  - Dual_Combined_Reg_Inj_Simultaneous/inc/stm32f10x_it.h    Interrupt handlers header file
  - Dual_Combined_Reg_Inj_Simultaneous/src/main.c            Main program
...
@par Hardware and Software environment
  ...
  - STM3210B-EVAL, STM3210E-EVAL and  STM3210C-EVAL Set-up 
    - Connect an external voltage (0-3.3v) to channel 8 (GPIOB.00). The source can be a power
      supply or a GBF (it's preferable to use 1kHz frequency to well recover the signal).
    - Connect an external voltage (0-3.3v) to channel 11 (GPIOC.01). The source can be a power
      supply or a GBF (it's preferable to use 1kHz frequency to well recover the signal).
    - Connect an external voltage (0-3.3v) to channel 12 (GPIOC.02). The source can be a power
      supply or a GBF (it's preferable to use 1kHz frequency to well recover the signal).
    ...
    - Connect a probe of an oscilloscope to GPIOA.11 to view the PWM signal generated
      by TIM1 to know when the injected channels are sampled.  
      
@par How to use it 

In order to load the Dual_Combined_Reg_Inj_Simultaneous project, you have to do the
following:
