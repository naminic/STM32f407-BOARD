/**
  @page ScanContinuous AN3116 ScanContinuous Readme file
 ...
 * @version  V1.0.0
 * @date     03/09/2010
 * @brief    Description of the AN3116 Application note's ScanContinuous.
 ********************************************************************************
...
This example describes how to use the ADC in Scan Continuous mode to convert 
successively and continuously a series of channels.
ADC1 converts successively four channels: external voltage in channel11, the internal 
reference voltage (VREFINT) channel17, the temperature sensor channel16 and the
(potentiometer) channel14.
Each channel has its own sampling time configuration. When ADC1 finishs the conversion
of channel14 it returns back to channel11 and so on.
The converted values are stored in ADC1ConvertedValue[] array.
...
@par Hardware and Software environment
  ...
  - STM3210B-EVAL, STM3210E-EVAL and  STM3210C-EVAL Set-up 
    - Connect an external voltage (0-3.3v) to channel11 (GPIOC.01). 
      Note: the peak to peak amplitude of the signal should be in the range of 0v to 3.3v.

@par How to use it 

In order to load the ScanContinuous project, you have to do the
following:
